...
CREATE TABLE VOLUME (ID SERIAL PRIMARY KEY, BOOK VARCHAR(13) NOT NULL REFERENCES BOOK(ISBN), FORMAT VARCHAR(50), OWNER INTEGER NOT NULL REFERENCES MEMBER(ID), INSERT_DATE DATE, LOAN_STATE VARCHAR(20), AVAILABLE_FOR_LOAN BOOLEAN, AVAILABLE_FOR_CONSULTATION BOOLEAN, SHELF INTEGER NOT NULL REFERENCES SHELF(ID), NOTES TEXT);
