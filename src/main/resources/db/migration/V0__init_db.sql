...
CREATE TABLE SHELF(ID SERIAL PRIMARY KEY, SHELF_NAME VARCHAR(50), SHELF_ADDRESS TEXT, NOTES TEXT);
CREATE TABLE MEMBER (ID SERIAL PRIMARY KEY, APPLICATION_USER_ID INTEGER NOT NULL REFERENCES APPLICATION_USER(ID), NAME VARCHAR(50), SURNAME VARCHAR(50), INS_DATE DATE, SHELF INTEGER NOT NULL REFERENCES SHELF(ID), TEL_NUMBER VARCHAR(12), EMAIL VARCHAR(100), ADDRESS TEXT, NOTES TEXT);
...
CREATE TABLE VOLUME (ID SERIAL PRIMARY KEY, BOOK VARCHAR(13) NOT NULL REFERENCES BOOK(ISBN), FORMAT VARCHAR(50), INSERT_DATE DATE, LOAN_STATE VARCHAR(20), AVAILABLE_FOR_LOAN BOOLEAN, AVAILABLE_FOR_CONSULTATION BOOLEAN, SHELF INTEGER NOT NULL REFERENCES SHELF(ID), NOTES TEXT);
